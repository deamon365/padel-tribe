    ...
    <section id="tarifs" className="bg-white py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <FadeIn className="text-center">
          ...
          <p className="mt-4 text-base text-tribe-black/50">
            Location de terrain — 1h30
          </p>
        </FadeIn>

        <FadeIn delay={0.1}>
          <div className="mx-auto mt-12 max-w-3xl overflow-hidden rounded-2xl border border-tribe-black/10">
            {/* Header */}
            <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 bg-tribe-black px-4 py-4 text-xs font-semibold text-white sm:grid-cols-3 sm:gap-x-0 sm:px-6 sm:text-sm">
              <div>Créneau</div>
            ...
            {pricingRows.map((row, i) => (
              <div
                ...
                className={`grid grid-cols-[1fr_auto_auto] gap-x-3 px-4 py-3 text-xs sm:grid-cols-3 sm:gap-x-0 sm:px-6 sm:py-4 sm:text-sm ${
                  i % 2 === 0 ? "bg-tribe-cream/50" : "bg-white"
                }`}
              >
                <div className="font-medium text-tribe-black">{row.slot}</div>
                <div className="w-16 text-center text-tribe-black/70 sm:w-auto">
                  {row.weekday}
                </div>
                <div className="w-16 text-center text-tribe-black/70 sm:w-auto">
                  {row.weekend}
          ...
          <p className="mx-auto mt-6 max-w-3xl text-center text-xs text-tribe-black/40">
