...
export function Advantages() {
  return (
    <section className="relative bg-tribe-dark py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <FadeIn className="text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold tracking-tight text-white md:text-5xl">
            Une expérience
            <br />
            <span className="text-tribe-gold">unique</span>
          </h2>
        </FadeIn>
        ...
        <StaggerContainer className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {advantages.map((item) => (
            <StaggerItem key={item.title}>
              <AnimatedCard className="group h-full rounded-2xl border border-white/10 bg-white/5 p-8 transition-all duration-300 hover:border-tribe-gold/30 hover:bg-white/10">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-tribe-green/20 transition-colors duration-300 group-hover:bg-tribe-green">
                  <item.icon className="h-6 w-6 text-tribe-gold transition-colors duration-300 group-hover:text-white" />
                </div>
